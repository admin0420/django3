...
class Choice(models.Model):
        ...
        #ForeignKey(연결할 다른 모델 클래스,on_delete=삭제방식)
        #Choice 모델클래스의 q변수는 연결된 Question객체와 동일함
        #-> Choice객체.q.title(또는 pub_date, id)변수를 접근할수있음
        #on_delete : 연결된 모델클래스의 객체가 삭제될때 어떻게처리 할지 지정하는 변수
        ...
        #modles.SET(연결할객체) : 연결된 객체가 삭제되면 매개변수로 넣은 객체와 연결
