...
<form> 태그 안에 <input> 태그를 작성해서 어떤 입력을 할 수 있는 지정할 수 있음
form태그의 속성으로 action과 method를 지정해야 함
action : 사용자의 입력을 웹서버의 어느 url로 보낼것인지 지정
method: Get 또는 POST 방식을 지정

장고의 form : HRML의 <form>태그에 들어가는 <input> 태그ㅏ들을
관리할 수 있는 클래스/기능
...
"""
ModelForm : 모델클래스를 기반으로 입력양식을 자동 생성할때 상속 받는 클래스
...
#Question 모델클래스와 연동된 모델폼클래스 저의
#Question 객체 추가/수정 때 사용
class QuestionForm(ModelForm):
    #Meta 클래스 : 연동하고자 하는 모델클래스에 대한 정보를
    #정의하는 클래스 (대문자 주의)
    class Meta:
            #이 모델폼클래스가 Question모델클래스와 연동
                model = Question
                #Question모델클래스의 변수중 title변수만 작성가능
                fields = ['title'] 
                #model, fields, exclude - 변수이름 주의
                #model : 연동하고자 하는 모델클래스를 지정하는 변수
                #fields : 모델클래스에 정의된 변수 중 어떤 변수를 클라이언트가 작성할 수 있는지 지정하는 변수
