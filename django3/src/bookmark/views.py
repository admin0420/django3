...
request : 웹클라이언트에 대한 정보(요청정보,<form>로 넘겨준 데이터,
            로그인 정보, 세션정보, 요청방식(GET,POST))를 저장한 변수
"""
...
def book_list(request):
    """
    모델클래스를 통해 데이터베이스 저장된 객체 가져오기
    Bookmark.objects.all() : 데이터베이스에 해당 모델클래스의
    객체를 모두 추출
    Bookmark.objects.get() : 데이터베이스에 특정조건을 만족하는
    객체 한개를 추출
    Bookmark.objects.filter() : 특정조건을 만족하는 모든 객체추출
    Bookmark.objects.exclude() : 특정조건을 만족하지않는 모든 객체 추출
    """
...
#뷰함수에서 매개변수를 추가할때마다 URLcomf의 URL설정 에서 추가설정을 해야함
def book_one(request, book_idx):
        ...
        return render(request, 'bookmark/book_one.hrml',
                                    { 'a' : obj })
